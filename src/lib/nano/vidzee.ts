...
const SERVERS: VidzeeServer[] = [
  ...
  { server: 4, sr: "4", name: "Nflix" },
  { server: 5, sr: "5", name: "Drag" },
];
...
async function getStreamKey(): Promise<string> {
  ...
  pendingKeyPromise = (async () => {
    try {
      ...
      if (!res || !res.ok) {
        failureCooldownUntil = Date.now() + 15000;
        return "";
      }
...
export async function fetchVidzee(
  ...
  for (const server of SERVERS) {
    const streams = await resolveVidzeeServer(server, key, id, season, episode);
    if (streams.length) {
      return streams[0];
    }
  }
