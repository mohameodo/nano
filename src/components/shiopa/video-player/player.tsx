...
export default function Player({
  ...
  const isHls = useCallback((url: string) => {
    return url.toLowerCase().includes(".m3u8") || url.toLowerCase().includes("/hls/") || url.includes("/api/proxy")
  }, [])
  ...
  // Prefer native nano player for proxied / HLS / direct media. Iframe only for rare external embeds.
  const forceNative =
    isDirect ||
    Boolean(isM3U8) ||
    embedUrl.includes("/api/proxy") ||
    /\.(m3u8|mp4|webm|mkv)(\?|$)/i.test(embedUrl)
